...
class DocumentRepository extends ServiceEntityRepository
{
    ...
    public function getStats(\App\Entity\User $user): array
    {
        ...
        $row = $conn->fetchAssociative(
            ...
            FROM document WHERE user_id = :uid',
            ['uid' => $userId]
        ) ?: [];

        $byType = $conn->fetchAllAssociative(
            'SELECT type_document, COUNT(*) AS cnt FROM document WHERE user_id = :uid GROUP BY type_document ORDER BY cnt DESC',
            ['uid' => $userId]
        );
        ...
        $byMonth = $conn->fetchAllAssociative(
            'SELECT DATE_FORMAT(created_at, "%Y-%m") AS month_key, DATE_FORMAT(created_at, "%b %Y") AS month_label, COUNT(*) AS cnt
             FROM document WHERE user_id = :uid AND created_at >= DATE_SUB(NOW(), INTERVAL 6 MONTH)
    ...
    public function getAdminStats(): array
    {
        ...
        $byType = $conn->fetchAllAssociative(
            'SELECT type_document, COUNT(*) AS cnt FROM document GROUP BY type_document ORDER BY cnt DESC'
        );

        $byMonth = $conn->fetchAllAssociative(
            'SELECT DATE_FORMAT(created_at, "%Y-%m") AS month_key, DATE_FORMAT(created_at, "%b %Y") AS month_label, COUNT(*) AS cnt
             FROM document WHERE created_at >= DATE_SUB(NOW(), INTERVAL 6 MONTH)
             GROUP BY month_key, month_label ORDER BY month_key ASC'
        );
